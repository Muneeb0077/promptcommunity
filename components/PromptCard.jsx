...
const PromptCard = ({prompt , handleTagClick,handleEdit,handleDelete,handleUsernameClick}) => {
      ...
      <div className="flex justify-between items-start gap-5">
        <div className="flex-1 flex justify-start items-center gap-3 cursor-pointer">
          ...
          <div className="flex flex-col">
            <h3 className="font-satoshi font-semibold text-white" onClick={()=>handleUsernameClick&&handleUsernameClick(prompt)}>{prompt.creator.username}</h3>
            <p className="font-inter text-sm text-gray-500">{prompt.creator.email}</p>
          </div>
        </div>
        <div className="copy_btn" onClick={handleCopy}>
          <Image src={copied === prompt.prompt ? '/assets/icons/tick.svg':'/assets/icons/copy.svg'} alt="tick" width={12} height={12} />
        </div>
      </div>
